...
-- Queried on every org-scoped audit page and compliance dashboard.
CREATE INDEX IF NOT EXISTS idx_audit_logs_org_id ON audit_logs(org_id);

--;;

-- build_events(build_id, created_at) — replace single-column index.
-- SSE event replay queries ORDER BY created_at, needs composite.
...
-- These tables support cursor-based pagination with ORDER BY (created_at DESC, id DESC).
