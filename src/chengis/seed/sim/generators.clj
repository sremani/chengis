...
(defn generate-sha
  ...
  [^Random rng]
  (let [sb (StringBuilder. 40)]
    (dotimes [_ 40]
      (.append sb (nth "0123456789abcdef" (next-int rng 16))))
    (str sb)))
...
(defn generate-webhook-event
  ...
  (let [provider "github"
        event-type (if (and (:branch git-info)
                            (or (.startsWith (:branch git-info) "feature/")
                                (.startsWith (:branch git-info) "fix/")))
                     "pull_request"
                     "push")]
    {:provider        provider
