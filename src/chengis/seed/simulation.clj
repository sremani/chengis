...
(defn- seed-users!
  ...
  (let [created-at (gen/format-ts (.atTime (gen/days-ago simulation-days) 8 0 0))]
    ...
    (loop [remaining  stages
      ...
      (when-let [stage-name (first remaining)]
        (let [is-fail-stage (and failed? (= stage-name fail-stage))
              ...
              stage-end     (.plusSeconds current-ts duration)
          ...
          (ins/insert-step! ds {:build-id     build-id
                                :stage-name   stage-name
                                :step-name    (str "run-" (.toLowerCase (.replace stage-name " " "-")))
                                :status       status
...
(defn- seed-single-build!
  ...
  (let [trigger-type  (gen/generate-trigger-type rng)
        ...
        total-secs    (reduce + (map (fn [s]
                                       (if (and fail-stage
                                                (let [idx-fail (.indexOf stages fail-stage)
                                                      idx-s    (.indexOf stages s)]
                                                  (> idx-s idx-fail)))
                                         0
                                         (gen/generate-stage-duration-seconds rng s)))
                                     stages))
        completed-ts  (.plusSeconds build-ts total-secs)
...
(defn- seed-job-audit-events!
  ...
  (let [admin-id  (get user-ids "admin")
        created-at (gen/format-ts (.atTime (gen/days-ago simulation-days) 8 30 0))]
